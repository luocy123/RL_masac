...
class World:
    def __init__(self, cfg: dict):
        ...
        start_x = cfg["red_uavs"]["start"]["x"]
        start_y = start_x
        self.spacing = cfg["red_uavs"]["spacing"]
        goal_x = cfg["red_uavs"]["goal"]["x"]
        goal_y = cfg["red_uavs"]["goal"]["y"]
        goal_radius = cfg["red_uavs"]["goal_radius"]
        speed = cfg["red_uavs"]["speed"]
        self.start_pos = self.assign_uav_square_formation(n=self.uav_num, start_x=start_x, start_y=start_y,
        ...
        for r in cfg["platforms"]["radars_moving"]:
            ...
            if r["mode"] == "patrol_points":
                ...
                # 获取边界信息，如果未提供则使用默认的地图边界
                bounds = r.get("bounds", [[0, self.W], [0, self.H]])
                bounds = np.array(bounds, dtype=float)
                initial_pos = np.array([r.get("initial_x", self.W/2), r.get("initial_y", self.H/2)], dtype=float)
                rm = RadarMoving(
                    id=r["id"], range=float(t["range"]), mode="random",
                    pos=initial_pos.copy(), init_pos=initial_pos.copy(),
                    bounds=bounds, random_speed=float(r.get("speed", 1.0))
                )
            else:  # circle
        ...
        for s in cfg["platforms"]["strikers_moving"]:
            ...
            if s["mode"] == "patrol_points":
                ...
                # 获取边界信息，如果未提供则使用默认的地图边界
                bounds = s.get("bounds", [[0, self.W], [0, self.H]])
                bounds = np.array(bounds, dtype=float)
                initial_pos = np.array([s.get("initial_x", self.W/2), s.get("initial_y", self.H/2)], dtype=float)
                sm = StrikerMoving(
                    id=s["id"],
                    home=initial_pos.copy(),
                    speed=float(s.get("speed", t["speed"])),
                    rng=float(t["range"]),
                    base_p=float(t["base_hit_prob"]),
                    mode="random",
                    pos=initial_pos.copy(),
                    init_pos=initial_pos.copy(),
                    bounds=bounds,
                    random_speed=float(s.get("speed", t["speed"]))
                )
            else:  # circle
    ...
    def step(self, actions, pre_states):
        ...
        # 1) 红方推进
        a_limit = self.uavs[0].a_limit
        v_limit = self.uavs[0].v_limit
        t = v_limit/a_limit
        for uav, action, state in zip(self.uavs, actions, pre_states):
            v_norm, v_angle, dis2edge, angle2edge, dis2obs, angle2obs, dis2radar, angle2radar, dis2striker, angle2striker, dis2other, angle2other, dis2target, angle2target = state[:14]
            if uav.find_obs:
                dis2obs = float(uav.find_obs[0][1] - uav.find_obs[0][0]["r"])
            if uav.find_striker:
                dis2striker = float(uav.find_striker[0][1] - uav.find_striker[0][0].rng)
            if dis2edge < a_limit*t*t:
                a_norm = uav.a_limit
                if angle2edge < 0:
                    a_angle = math.pi + angle2edge
                else:
                    a_angle = angle2edge - math.pi
            elif dis2obs < a_limit*t*t:
                a_norm = uav.a_limit
                if angle2obs < 0:
                    a_angle = math.pi + angle2obs
                else:
                    a_angle = angle2obs - math.pi
            elif dis2striker < a_limit*t*t:
                a_norm = uav.a_limit
                if angle2striker < 0:
                    a_angle = math.pi + angle2striker
                else:
                    a_angle = angle2striker - math.pi
            else:
                if abs(v_angle - angle2target) > 0.1:
                    a_norm = action[0]
                    a_angle = angle2target
                else: 
                    a_norm, a_angle = action
           
            uav.control(a_norm*5, a_angle)
            uav.update(self.dt)
        # 2) 雷达推进（移动雷达位置）
        ...
        rewards, dones = self.get_rewards(pre_states=pre_states, cur_states=cur_states, env_size=self.W)
        ...
        for pre_state, cur_state, uav in zip(pre_states, cur_states, self.uavs):
            ...
            if uav.dead_flag:
                ...
                if cur_dis2target < uav.goal_radius:
                    ...
                    uav.arrival_time = self.tick*self.dt  # 记录到达时间
                ...
                if pre_dis2target < cur_dis2target:
                    ...
                    if (pre_dis2target-cur_dis2target) > 0.6:
                        reward += 10.0
                # 目标角度奖励
